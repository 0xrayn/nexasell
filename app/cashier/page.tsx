  ...
  const CartPanel = () => (
    <div className="flex flex-col h-full bg-[var(--surface)]">
      ...
      <div className="border-t border-[var(--border)] p-4 space-y-4 flex-shrink-0 bg-[var(--surface2)]">
          ...
          <div className="space-y-2">
            ...
            {cashNum > 0 && total > 0 && (
              <div className={`flex justify-between items-center p-3 rounded-xl font-bold text-sm ${
                change >= 0 ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20" : "bg-red-50 dark:bg-red-500/10 text-red-600 border border-red-200 dark:border-red-500/20"
              }`}>
        ...
        {payMethod !== "cash" && (
          <div className="p-3 bg-blue-50 dark:bg-blue-500/10 rounded-xl border border-blue-200 dark:border-blue-500/20 text-center">
            <p className="text-xs text-blue-600 dark:text-blue-400 font-bold">
              {payMethod === "card" ? "💳 Tap atau gesek kartu" : "📱 Scan QR code e-wallet"}
            </p>
          </div>
        )}

        <button onClick={handlePay} disabled={!canPay}
          className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black text-sm transition-all active:scale-95 shadow-lg shadow-emerald-500/30 flex items-center justify-center gap-2">
          <Receipt className="w-4 h-4" />
          {cart.length === 0 ? "Pilih Produk Dulu" : `Bayar ${formatRupiah(total)}`}
        </button>
      </div>
    </div>
  );
                ...
                <button key={p.id} onClick={() => addToCart(p)}
                  className={`relative text-left bg-[var(--surface)] rounded-2xl border-2 transition-all hover:shadow-lg active:scale-95 overflow-hidden ${
                    inCart
                      ...
                      : "border-transparent shadow-sm hover:border-[var(--text2)]/20"
                  }`}>
      ...
      {paid && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md px-4">
          <div className="bg-[var(--surface)] rounded-3xl p-7 max-w-xs w-full text-center shadow-2xl border border-[var(--border)] anim-slide-up">
            <div className="w-20 h-20 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-3xl flex items-center justify-center mx-auto mb-5 shadow-xl shadow-emerald-500/30 anim-float">
            ...
            <div className="bg-[var(--surface2)] rounded-2xl p-4 mb-5 space-y-2.5 border border-[var(--border)] text-left">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold capitalize text-[var(--text)]">{payMethod}</span>
              </div>
              {payMethod === "cash" && (
            ...
            <button onClick={handleReset}
              className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-black text-sm flex items-center justify-center gap-2 transition-all active:scale-95">
