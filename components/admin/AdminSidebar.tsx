...
function SidebarContent({ isMobile = false }: { isMobile?: boolean }) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background:"var(--surface)", borderRight:"1px solid var(--border)" }}>
      ...
      {/* Header — logo always visible, toggle always visible */}
      <div className="flex items-center h-16 px-3 gap-2.5 flex-shrink-0" style={{ borderBottom:"1px solid var(--border)" }}>
        ...
        {/* Collapse toggle — inline, always rendered */}
        {!isMobile && (
          <button onClick={toggle}
            className="flex-shrink-0 w-7 h-7 rounded-xl flex items-center justify-center transition-all"
            style={{ background:"var(--surface2)", border:"1px solid var(--border)", marginLeft:"auto" }}
      ...
      {/* User footer with dropdown — only when expanded */}
      <div className="px-2 pb-3 flex-shrink-0" style={{ borderTop:"1px solid var(--border)", paddingTop:8 }}>
        {!isCol ? (
          ...
          /* Collapsed: just show avatar, clicking goes to settings */
          <Link href="/admin/settings"
            className="flex justify-center p-2 rounded-2xl transition-all"
            onMouseEnter={e=>(e.currentTarget.style.background="var(--surface2)")}
            onMouseLeave={e=>(e.currentTarget.style.background="")}>
...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className="hidden lg:flex flex-col fixed left-0 top-0 h-screen z-30 sidebar-transition overflow-hidden"
